...
public class CaseRunnerGenerator {
	...
	public int loadCaseRunnerClasses() {
		...
		Iterator<Entry<String, String>> iter = caseRunnerJavaFileNames.entrySet()
				.iterator();
		int retVal = 0;//NOPMD
		while (iter.hasNext()) {
	...
	private void loadClass(String classFilePathName, String className)
		...
		optionList.add( getAllJarsClassPathInMavenLocalRepo() + System.getProperty("java.class.path")
		...
		if (task.call()) {
			...
			URLClassLoader classLoader = new URLClassLoader(
					new URL[] { new File(System.getProperty("user.dir")
							+ "/generated-code/caserunners/").toURI().toURL() }, Thread.currentThread().getContextClassLoader());
