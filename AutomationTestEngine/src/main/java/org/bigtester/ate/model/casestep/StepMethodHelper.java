...
public class StepMethodHelper {
      ...
      boolean unique, List<ITestNGMethod> outExcludedMethods)
  {
    ...
    StepMethodGroupsHelper.collectMethodsByGroup(methods.toArray(new ITestNGMethod[methods.size()]),
        forTests,
        includedMethods,
        outExcludedMethods,
        runInfo,
    ...
    for (ITestNGMethod m : methods) {
      ...
      if (methodsDependedUpon.length > 0) {
        ITestNGMethod[] methodsNamed =
          StepMethodHelper.findDependedUponMethods(m, methods);
      ...
      if (groupsDependedUpon.length > 0) {
        for (String group : groupsDependedUpon) {
          ITestNGMethod[] methodsThatBelongToGroup =
            StepMethodGroupsHelper.findMethodsThatBelongToGroup(m, methods, group);
